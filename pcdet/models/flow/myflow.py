...
class MyFlow(torch.nn.Module):
    ...
    def GRU_based_refinement(self, sp2p_flow, C, pc0, pc1, feat0, feat1, graph, pc0_ori, h, iter):
        ...
        flow0_up = self.upsample(pc0_ori.permute(0, 2, 1), pc0.permute(0, 2, 1), sp2p_flow.permute(0, 2, 1)).permute(0,
                                                                                                                     2,
                                                                                                                     1)
        return sp2p_flow, flow0_up, h

    def confidence_encoding(self, prototypes_src_flow, prototypes_tgt_flow, c2p_idx_abs0, c2p_idx_abs1, a0, a1, pc0,
                            pc1):
        ...
        sp2p_flow0 = torch.sum(c2p_flow0 * a0.unsqueeze(1), dim=-1, keepdim=False).transpose(1,
                                                                                             2).contiguous()  # [b,n,3]
        ...
        # (b, 12, m), (b, n, n2cp) -> b x 12 x n x n2cp
        sp2p_flow1 = torch.sum(c2p_flow1 * a1.unsqueeze(1), dim=-1, keepdim=False).transpose(1,
                                                                                             2).contiguous()  # [b,n,3]
    ...
    def obtain_clustering(self, pc0, pc1, feat0, feat1, ot_flow, ot_flow_back, flows0, flows1, pc0_ori, pc1_ori, graph0,
                          graph1, \
        ...
        for iter in range(self.num_iters):
            a0, f0, z0, prototypes_src_feat, prototypes_src_xyz, prototypes_src_flow, sp_numc0 = self.learn_SLIC_calc_1(
                prototypes_src_feat, prototypes_src_xyz, prototypes_src_flow, \
                feat0, pc0, ot_flow, c2p_idx_abs0, c2p_idx0, cluster_idx0, prototypes_src_fused_xyz,
                prototypes_src_fused_feat, fused_xyz_0, fused_feats_0)  # a [b,n,nc2p]# z [b,m,n] l1_norm/dim2

            a1, f1, z1, prototypes_tgt_feat, prototypes_tgt_xyz, prototypes_tgt_flow, sp_numc1 = self.learn_SLIC_calc_1(
                prototypes_tgt_feat, prototypes_tgt_xyz, prototypes_tgt_flow, \
                feat1, pc1, ot_flow_back, c2p_idx_abs1, c2p_idx1, cluster_idx1, prototypes_tgt_fused_xyz,
                prototypes_tgt_fused_feat, fused_xyz_1, fused_feats_1)  # a [b,n,nc2p]# z [b,m,n] l1_norm/dim2

            sp2p_flow0, sp2p_flow1, C0, C1 = self.confidence_encoding(prototypes_src_flow, prototypes_tgt_flow,
                                                                      c2p_idx_abs0, c2p_idx_abs1, a0, a1, pc0, pc1)

            ot_flow, flow0_up, h0 = self.GRU_based_refinement(sp2p_flow0, C0, pc0, pc1, feat0, feat1, graph0, pc0_ori,
                                                              h0, iter)
            flows0.append(flow0_up)
            ot_flow_back, flow1_up, h1 = self.GRU_based_refinement(sp2p_flow1, C1, pc1, pc0, feat1, feat0, graph1,
                                                                   pc1_ori, h1, iter)
            ...
            prototypes_src_fused_xyz, prototypes_tgt_fused_xyz, prototypes_src_fused_feat, prototypes_tgt_fused_feat = \
                self.feature_coordinate_space(prototypes_src_xyz, prototypes_tgt_xyz, prototypes_src_flow,
                                              prototypes_tgt_flow, \
                                              prototypes_src_feat, prototypes_tgt_feat, prototypes_sa_feats_0,
                                              prototypes_sa_feats_1)

            # at point-level
            sa_feats_0 = self.obtain_feat(pc0, pc1, feat1, ot_flow)
            sa_feats_1 = self.obtain_feat(pc1, pc0, feat0, ot_flow_back)
            fused_xyz_0, fused_xyz_1, fused_feats_0, fused_feats_1 = self.feature_coordinate_space(pc0, pc1, ot_flow,
                                                                                                   ot_flow_back, \
                                                                                                   feat0, feat1,
                                                                                                   sa_feats_0,
                                                                                                   sa_feats_1)
    ...
    def process(self, pc0, pc1, npoint, nsample):
        ...
        fused_xyz_0, fused_xyz_1, fused_feats_0, fused_feats_1 = self.feature_coordinate_space(pc0_d, pc1_d,
                                                                                               ot_flow_forward,
                                                                                               ot_flow_backward, \
                                                                                               feats_0, feats_1,
                                                                                               sa_feats_0, sa_feats_1)

        up_flow0 = self.upsample(pc0.permute(0, 2, 1), pc0_d.permute(0, 2, 1),
                                 ot_flow_forward.permute(0, 2, 1)).permute(0, 2, 1)
        flows0.append(up_flow0)
        up_flow1 = self.upsample(pc1.permute(0, 2, 1), pc1_d.permute(0, 2, 1),
                                 ot_flow_backward.permute(0, 2, 1)).permute(0, 2, 1)
        flows1.append(up_flow1)

        flows0, flows1 = self.obtain_clustering(pc0_d, pc1_d, feats_0, feats_1, ot_flow_forward, ot_flow_backward, \
                                                flows0, flows1, pc0, pc1, graph0, graph1, fused_xyz_0, fused_xyz_1,
                                                fused_feats_0, fused_feats_1, num_c=self.numc)
        return flows0, flows1
    ...
    def forward(self, batch_dict):
        ...
        voxels, coordinates, voxel_num_points = batch_dict['voxels'], batch_dict['voxel_coords'], batch_dict[
            'voxel_num_points']
        num_voxel1 = (coordinates[:, 1] == 0).sum()
        num_voxel2 = (coordinates[:, 1] == 0).sum()
        voxels1 = voxels[coordinates[:, 1] == 0]
        voxels2 = voxels[coordinates[:, 1] == 1]
        voxel_num_points1 = voxel_num_points[coordinates[:, 1] == 0]
        voxel_num_points2 = voxel_num_points[coordinates[:, 1] == 1]
