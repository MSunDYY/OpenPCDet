...
from ..model_utils.denet_utils import PointNet, MLP, SpatialMixerBlock, build_voxel_sampler_denet,build_transformer
from ..model_utils.denet3_utils import unflatten, SpatialMixerBlockCompress, CrossMixerBlock
...
class ProposalTargetLayerMPPNet1(ProposalTargetLayer):
    ...
    def forward(self, batch_dict):
        ...
        batch_rois, batch_gt_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels, \
            batch_backward_rois, batch_trajectory_rois, batch_valid_length = self.sample_rois_for_mppnet(
...
class DENetHead(RoIHeadTemplate):
    def __init__(self, model_cfg, num_class=1, **kwargs):
        ...
        # self.compress_points = nn.Sequential(nn.Conv1d(256*self.grid_size**3,512k,kernel_size=1),
        #                                      nn.BatchNorm1d(512),
        #                                      nn.ReLU(),
        #                                      nn.Dropout(0.2),
    ...
    def points_features_pool(self, roi_boxes, query_points, query_boxes_idx, num_sample, ):
        ...
        for idx in range(query_boxes_idx.shape[0] - 1):
            ...
            dis = torch.norm(
                (cur_points[:, :2].unsqueeze(0) - cur_boxes[:, :2].unsqueeze(1).repeat(1, cur_points.shape[0], 1)),
                dim=2)
    ...
    def get_proposal_aware_geometry_single_feature(self, src, batch_size, rois, num_rois):
        ...
        corner_add_center_points = torch.cat([corner_points, trajectory_roi_center], dim=-1)
        proposal_aware_feat = src[:, :, :3].repeat(1, 1, 9) - corner_add_center_points.unsqueeze(1).repeat(1,
                                                                                                           self.num_lidar_points,
                                                                                                           1)
    ...
    def get_proposal_aware_geometry_feature(self, src, batch_size, rois, num_rois):
        ...
        # time_stamp = torch.concat(time_stamp,1)
        # proposal_aware_feat = torch.cat(proposal_aware_feat_list, dim=1)
        proposal_aware_feat = torch.cat([proposal_aware_feat, src], dim=-1)
    ...
    def trajectories_auxiliary_branch(self, trajectory_rois):
        ...
        time_stamp = torch.ones(
            [trajectory_rois.shape[0], trajectory_rois.shape[1],1]).cuda()
        ...
        ).view(box_seq.shape[0],-1,  box_seq.shape[-1])
        ...
        box_reg, box_feat, _ = self.seqboxembed(
            box_seq.permute(1,2,0).contiguous())
    ...
    def assign_stack_targets(self, points, gt_boxes, points_bs_idx, extend_gt_boxes=None,
        ...
        extend_gt_boxes = box_utils.enlarge_box3d(gt_boxes.view(-1, gt_boxes.shape[-1]),
                                                  extra_width=[0.2, 0.2, 0.2]).unflatten(0, (gt_boxes.shape[0], -1))
    ...
    def forward(self, batch_dict):
        ...
        :return:
        """
        num_frames = self.model_cfg.Transformer.num_frames_all
        ...
        if self.training:
            ...
            batch_dict['num_rois'] = targets_dict['num_rois']
        else:
            empty_mask = batch_dict['roi_boxes'][:, :, :6].sum(-1) == 0
        ...
        if False:
            with torch.no_grad():
                ...
                num_rois = torch.tensor([0] + [(roi_list[i, :, 0] != 0).sum() for i in range(roi_list.shape[0])],
                                        device=device)

                roi_list = torch.concat([roi_list[i, :num_rois[i + 1]] for i in range(roi_list.shape[0])])
                num_rois = torch.cumsum(num_rois, dim=0)
                src_pre, query_points_features_pre, query_points_bs_idx_pre = self.voxel_sampler(batch_size, roi_list,
                                                                                                 num_sample, batch_dict,
                                                                                                 start_idx=1,
                                                                                                 num_rois=num_rois)
                ...
                batch_dict['query_points_bs_idx1'] = query_points_bs_idx_pre
                ...
                src_pre = self.get_proposal_aware_geometry_single_feature(src_pre, batch_size * (num_frames - 1),
                                                                          roi_list,
                                                                          num_rois)
            ...
            query_points_bs_idx_pre = batch_dict['query_points_bs_idx3'].detach()
            query_points_bs_idx_pre = F.pad(torch.cumsum(query_points_bs_idx_pre, dim=0), (1, 0), value=0)

            for idx in range(num_frames - 1):
                for bs in range(batch_size):
                    query_points_single = query_points_features_pre[
                                          query_points_bs_idx_pre[idx * batch_size + bs]:query_points_bs_idx_pre[
                                              idx * batch_size + bs + 1]]
                    query_points_single[:, :3] = torch.matmul(F.pad(query_points_single[:, :3], (0, 1, 0, 0), value=1),
                                                              batch_dict['poses'][bs, 4 * (idx + 1):4 * (idx + 2)].t())[
                                                 :, :3]
                    query_points_single[:, :3] = torch.matmul(F.pad(query_points_single[:, :3], (0, 1, 0, 0), value=1),
                                                              batch_dict['poses'][bs, :4].t().inverse())[:, :3]
                    query_points_features_pre[query_points_bs_idx_pre[idx * batch_size + bs]:query_points_bs_idx_pre[
                        idx * batch_size + bs + 1]] = query_points_single
        ...
        src, points_index = self.voxel_sampler(batch_size, roi_boxes, num_sample,
        ...
        # src_motion_feature = self.get_proposal_aware_motion_feature(src, batch_size, roi_boxes, num_rois)
        ...
        hs, tokens = self.transformer(src,batch_dict, pos=None)
