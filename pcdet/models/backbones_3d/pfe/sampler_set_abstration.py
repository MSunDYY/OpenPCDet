...
class SamplerSetAbstraction(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        for src_name in self.model_cfg.FEATURES_SOURCE:
            ...
            cur_layer, cur_num_c_out = pointnet2_stack_modules.build_local_aggregation_module(
                input_channels=input_channels, config=SA_cfg[src_name],use_xyz=False
            )
        ...
        self.classfier = nn.Linear(self.model_cfg.NUM_OUTPUT_FEATURES,2,bias=False)
    ...
    def forward(self, batch_dict):
        ...
        for k, src_name in enumerate(self.SA_layer_names):
            ...
            xyz = points[:, 1:4]
            xyz_features = points[:,1:]
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                batch_size=batch_size, aggregate_func=self.SA_layers[k],
                xyz=xyz.contiguous(), xyz_features=xyz_features, xyz_bs_idxs=points[:, 0],
                new_xyz=new_xyz, new_xyz_batch_cnt=new_xyz_batch_cnt,
        ...
        batch_dict['point_coords'] = keypoints[:,:4]  # (BxN, 4)
        return batch_dict
