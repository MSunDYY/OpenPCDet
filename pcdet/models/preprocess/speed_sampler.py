...
class SpeedSampler(nn.Module):
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        frame_num = int(batch_dict['points'][:, -1].max() * 10) + 1
        voxel_generator = VoxelGeneratorWrapper(
            vsize_xyz=self.voxel_size,
            coors_range_xyz=self.point_cloud_range,
            num_point_features=batch_dict['points'].shape[1]-1 ,
        ...
        for batch in range(batch_dict['batch_size']):
            points = batch_dict['points'][:,1:][batch_dict['points'][:,0]==batch]
            for frame in range(frame_num):
                ...
                coordinate = np.concatenate([batch*np.ones((coordinate.shape[0],1)),frame * np.ones((coordinate.shape[0], 1)), coordinate], axis=1)
        ...
        # voxels, voxel_num_points, coordinates = batch_dict['flow_voxels'], batch_dict['flow_num_points'], batch_dict[
        #     'flow_coords']
        ...
        if self.use_absoluto_xyz:
            features = [voxels[:, :, :6], f_cluster, f_center]
        else:
            features = [voxels[:, :, :6], f_cluster]
        ...
        pred_speed = self.regression(pillar_map)
        pred_speed = torch.permute(pred_speed.reshape(B, H, W, -1),(0,3,1,2))
