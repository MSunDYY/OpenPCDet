...
def boxes_iou3d_cpu(boxes_a, boxes_b):
    ...
    sa = boxes_a[:,3]*boxes_a[:,4]
    sb = boxes_b[:,3]*boxes_b[:,4]

    iou_bev = boxes_a.new_zeros(torch.Size((boxes_a.shape[0], boxes_b.shape[0])))
    iou3d_nms_cuda.boxes_iou_bev_cpu(boxes_a.contiguous(), boxes_b.contiguous(), iou_bev)
    overlaps_bev = (sa[:,None]+sb[None,:])*iou_bev/(1+iou_bev)
    ...
    return iou3d.numpy() if is_numpy else iou3d
