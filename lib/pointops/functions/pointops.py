...
class KNNQuery(Function):
    @staticmethod
    def forward(ctx, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        KNN Indexing
        input: nsample: int32, Number of neighbor
               xyz: (b, n, 3) coordinates of the features
...
class ThreeNN(Function):
    ...
    def forward(ctx, unknown: torch.Tensor, known: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        B, N, _ = unknown.size()
        m = known.size(1)
        dist2 = torch.cuda.FloatTensor(B, N, 3).zero_()
        idx = torch.cuda.IntTensor(B, N, 3).zero_()
        pointops_cuda.three_nn_wrapper(B, N, m, unknown, known, dist2, idx)
        return torch.sqrt(dist2), idx
...
class AssoMatrixPlusLabel(Function):
    ...
    def forward(ctx, ks: int, idx_c: torch.Tensor, cid: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        idx = torch.cuda.IntTensor(b, m, n).zero_()
        cnt = torch.cuda.IntTensor(b, m, 1).zero_()
        pointops_cuda.assomatrix_label_cuda(b, n, m, ks, idx_c, cid, idx, cnt)
        return idx, cnt
